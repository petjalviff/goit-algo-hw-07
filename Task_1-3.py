...
class AVLNode:
    ...
    def sum_of_values(self):
        if self is None:
            return 0
        return self.key + (self.left.sum_of_values() if self.left else 0) + (self.right.sum_of_values() if self.right else 0)
...
def gen_keys(size):
    return [round(random.randint(1, 1000)) for _ in range(size)]
